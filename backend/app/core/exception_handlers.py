...
from core.exceptions import BaseAppException, InvalidVerifyToken,UserNotExists
...
def invalid_verify_token_handler(request: Request,exc: InvalidVerifyToken):
    ...
    return JSONResponse(status_code=exc.http_status, content=error_resp.model_dump(by_alias=True, exclude_none=True))
...
def global_exception_handler(request:Request,exc: BaseAppException):
    ...
    error_resp = ErrorResponse(
        status=exc.error_status,
        message=exc.message,
        error=Error(code=500, details=exc.detail),
        meta=Meta(timestamp=_now()),
        )
    return JSONResponse(status_code=exc.http_status, content=error_resp.model_dump(by_alias=True, exclude_none=True))
